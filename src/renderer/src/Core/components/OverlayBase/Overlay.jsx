...
const Overlay = ({ showOverlay, setShowOverlay, children }) => {
  return (
    <div className={`fixed inset-0 ${showOverlay ? 'block' : 'hidden'} bg-black bg-opacity-50 z-50 flex justify-center items-center`}>
      <div className="bg-gray-800 rounded-lg shadow-lg p-8 w-full max-w-md bg-slate-600">
        {/* Close button */}
        <button className="absolute top-0 right-0 p-2 focus:outline-none" onClick={() => setShowOverlay(false)}>
